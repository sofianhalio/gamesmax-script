...
def log_replace(value, status='CARI APA', color='[Y1]'):
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='CARI APA', color='[Y1]'):
    ...
    def run(self):
        try:
            ...
            self.log('JANDA BOLONG'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('JANDA MUDA ''[R2]'.format(self.proxy_host, self.proxy_port), color='[R1]')
        except OSError:
